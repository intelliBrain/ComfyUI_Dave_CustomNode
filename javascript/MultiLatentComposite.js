...
function addMultiLatentCompositeCanvas(node, app) {
  function findSizingNode(node, index = null) {
    const inputList =
      index !== null ? [index] : [...Array(node.inputs.length).keys()];
    ...
    for (let i of inputList) {
      ...
      if (connectedNodes.length !== 0) {
        ...
        // if previous connection is broken
  ...
  function getSizeFromNode(node) {
    ...
    for (let widget of node.widgets) {
      if (widget.name.toLowerCase() === "width") {
        endWidth = widget.value;
      }
      if (widget.name.toLowerCase() === "height") {
        endHeight = widget.value;
      }
    }
  ...
  const widget = {
    ...
    draw: function (ctx, node, widgetWidth, widgetY) {
      ...
      if (node.sampleToID && node.graph._nodes_by_id[node.sampleToID]) {
        [width, height] = getSizeFromNode(
          node.graph._nodes_by_id[node.sampleToID]
        );
      }
      ...
      if (width && height) {
        ...
        ctx.arc(
          ...
          LiteGraph.NODE_SLOT_HEIGHT * (index + 1.5) + 4,
          4,
          0,
          Math.PI * 2
        );
      ...
      if (node.selected) {
        ...
        if (selectedNode) {
          ...
          const [x, y, w, h] =
            node.graph._nodes_by_id[selectedNode].getBounding();
          const offset = 5;
...
app.registerExtension({
  name: "Comfy.ib2k24.MultiLatentComposite",
    ...
    if (nodeData.name === "MultiLatentComposite") {
      ...
      nodeType.prototype.onNodeCreated = function () {
        ...
        CUSTOM_INT(
          ...
          { min: 0.0, max: 4096, step: 80, precision: 0 }
        );
