  ...
  const widget = {
    ...
    draw: function (ctx, node, widgetWidth, widgetY) {
      ...
      const scale = Math.min(
        (widgetWidth - margin * 2) / width,
        (widgetHeight - margin * 2) / height
      );
      ...
      ctx.fillRect(
        widgetX - border,
        widgetY - border,
        backgroudWidth + border * 2,
        backgroundHeight + border * 2
      );
      ...
      ctx.arc(
        ...
        LiteGraph.NODE_SLOT_HEIGHT * (index + 0.5) + 4,
        4,
        0,
        Math.PI * 2
      );
      ...
      if (node.selected) {
        const connectedNodes = recursiveLinkUpstream(
          node,
          ...
          for (let [node_ID, depth] of connectedNodes) {
            ...
            if (connectedNode.type != node.type) {
              ...
              const titleHeight =
                ...
                (connectedNode.type === "Reroute" ? 0 : 1);

              ctx.strokeStyle = selectedColor;
              ctx.lineWidth = 5;
              ctx.strokeRect(
                x - offset - node.pos[0],
                y - offset - node.pos[1] - titleHeight,
                w + offset * 2,
                h + offset * 2 + titleHeight
              );
            }
          }
        }
...
app.registerExtension({
  name: "Comfy.ib2k24.MultiAreaConditioning",
  async beforeRegisterNodeDef(nodeType, nodeData, app) {
    if (nodeData.name === "MultiAreaConditioning") {
      ...
      nodeType.prototype.onNodeCreated = function () {
        ...
        CUSTOM_INT(this, "resolutionX", 512, function (v, _, node) {
          const s = this.options.step / 10;
          this.value = Math.round(v / s) * s;
          node.properties["width"] = this.value;
        });
        CUSTOM_INT(this, "resolutionY", 512, function (v, _, node) {
          const s = this.options.step / 10;
          this.value = Math.round(v / s) * s;
        ...
        CUSTOM_INT(this, "x", 0, function (v, _, node) {
          transformFunc(this, v, node, 0);
        });
        CUSTOM_INT(this, "y", 0, function (v, _, node) {
          transformFunc(this, v, node, 1);
        });
        CUSTOM_INT(this, "width", 0, function (v, _, node) {
          transformFunc(this, v, node, 2);
        });
        CUSTOM_INT(this, "height", 0, function (v, _, node) {
          transformFunc(this, v, node, 3);
        });
        CUSTOM_INT(
          this,
          "strength",
          1,
          function (v, _, node) {
            transformFunc(this, v, node, 4);
          },
          { min: 0.0, max: 10.0, step: 0.1, precision: 2 }
        );

        this.getExtraMenuOptions = function (_, options) {
          options.unshift(
            {
              content: `insert input above ${
                this.widgets[this.index].value
              } /\\`,
              callback: () => {
            ...
            },
            {
              ...
              callback: () => {
                ...
                if (index !== 0) {
                  ...
                  this.properties["values"].splice(
                    index - 1,
                    0,
                    this.properties["values"].splice(index, 1)[0]
                  );
  ...
  loadedGraphNode(node, _) {
    if (node.type === "MultiAreaConditioning") {
      node.widgets[node.index].options["max"] =
        node.properties["values"].length - 1;
    }
  },
});
